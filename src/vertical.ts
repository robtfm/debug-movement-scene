...
function applyGravity(dt: number) {
  Vector3.addToRef(velocity, Vector3.scale(GRAVITY, dt), velocity);
  if (grounded) {
    Vector3.addToRef(velocity, Vector3.scale(GRAVITY_DIR, Math.min(0.0, Vector3.dot(velocity, Vector3.scale(GRAVITY_DIR, -1)))), velocity);
  }
}
