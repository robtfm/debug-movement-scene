...
import { GROUND_SNAP_HEIGHT, GROUNDED_ANGLE, GROUNDED_HEIGHT, PLAYER_COLLIDER_RADIUS, VEC3_NEG_INF, VEC3_ZERO } from './constants';
...
export function initGroundRaycast() {
    ...
    const contactMarker = engine.addEntity();
    Transform.create(contactMarker, { scale: {x: 0.1, y: 0.1, z: 0.1}});
    MeshRenderer.setSphere(contactMarker);
    Material.setBasicMaterial(contactMarker, { diffuseColor: Color4.Red()});
    VisibilityComponent.create(contactMarker, { visible: false });
    ...
    raycastSystem.registerGlobalDirectionRaycast({
        ...
        (hit) => {
            ...
            VisibilityComponent.createOrReplace(contactMarker, { visible: hit.hits.length > 0 })
            ...
            if (hit.hits.some((hit) => {
                ...
                Vector3.copyFrom(hit.position ?? VEC3_ZERO, Transform.getMutable(contactMarker).position);
                Vector3.copyFrom(hit.position ?? VEC3_NEG_INF, groundPosition);
                Material.setBasicMaterial(contactMarker, { diffuseColor: groundTest ? Color4.Blue() : Color4.Red() });
...
function recordGroundState() {
    ...
    if (playerPosition.y < 0.01) {
        Vector3.fromFloatArrayToRef([0, 1, 0], 0, groundNormal);
    }
    if (grounded) {
        lastGroundTime = time;
    } else {
        Vector3.fromFloatArrayToRef([0, 0, 0], 0, groundNormal);
    }
}
